use ash::vk::{DescriptorBufferInfo, DescriptorPool, DescriptorPoolCreateInfo, DescriptorPoolSize, DescriptorSet, DescriptorSetAllocateInfo, DescriptorSetLayout, DescriptorSetLayoutBinding, DescriptorSetLayoutCreateInfo, DescriptorType, DeviceSize, ShaderStageFlags, WriteDescriptorSet};
...
impl Descriptor{
    pub fn new(device:&Device) -> Self{
        ...
        let layout = unsafe{device.logical.create_descriptor_set_layout(&layout_create_info,None)}
            .expect("Could not create Descriptor Set Layout");
        Self{ layout,pool:None,sets:None}
    }
    ...
    pub fn create_descriptor_sets(&mut self, device: &Device,buffers: &Buffers){
        ...
        let pool = unsafe{device.logical.create_descriptor_pool(&descriptor_pool_create_info,None)}
        ...
        let descriptor_sets = unsafe{device.logical.allocate_descriptor_sets(&descriptor_set_allocate_info)}
        ...
        for i in 0 .. buffer_count{
            ...
            unsafe{device.logical.update_descriptor_sets(&[write_descriptor_set],&[])}
        }
        self.pool = Some(pool);
